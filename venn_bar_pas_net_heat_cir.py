...
def pastel(df = DataFrame([]), column = 0, dim = 111, title = 'Title',
           ...
           size_text = 10, size_title = 20, color = pie_colors['Set3'],
           explode = 0, open_center = 0.4):
    if len(df) < 1:
        print('Data frame sin datos')
    else:
        explo = explode
        porcentaje = np.round((list(df.iloc[0:row_num,3]) / df.iloc[0:row_num,3].sum()) * 100, 1)
        labb = list(df.iloc[0:row_num,column])
        labels_box = [i+' ('+str(j)+'%)' for i, j in zip(labb, porcentaje)]
        labels_lab = list(df.iloc[0:row_num,column])
        ...
        explode0 = np.repeat(explode, row_num)
