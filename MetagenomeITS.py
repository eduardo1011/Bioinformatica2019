...
os.makedirs('PlotsITS', exist_ok = True)
...
def update_plot(mostrar_dendrograma = True, mostrar_circulos = True, mostrar_metadata = True,
    ...
    ########################## archivos
    sumary111 = []
    uno = open('AnexosITS/SUMARY2.txt', 'r')
    ...
    names111 = []
    uno = open('AnexosITS/names1.txt', 'r')
    for enu, line in enumerate(uno):
        line = line.rstrip()
        if enu == 0:
            header = line.split('\t')
    ...
    with open('AnexosITS/dend.json', 'r') as fp:
        dendro = json.load(fp)
    with open('AnexosITS/ssss.json', 'r') as fp:
        ssss = json.load(fp)
    with open('AnexosITS/dictionaries.json', 'r') as fp:
        oooo = json.load(fp) 
...
def update_plot2(mostrar_dendrograma = True, mostrar_circulos = True, mostrar_metadata = True,
    ...
    names111 = []
    uno = open('AnexosITS/names111.txt', 'r')
    for enu, line in enumerate(uno):
        line = line.rstrip()
        if enu == 0:
            header = line.split('\t')
    ...
    with open('AnexosITS/dendo.json', 'r') as fp:
        dendro = json.load(fp)
    with open('AnexosITS/numeros2.json', 'r') as fp:
        numeros2 = json.load(fp)
    with open('AnexosITS/dictionaries2.json', 'r') as fp:
        oooo = json.load(fp) 
    variedad = oooo[0]
    procesado = oooo[1]
    cultivo = oooo[2]
...
def box1(data, Percentage, Method, Metric, Rampas,
    ...
    SUMARY2.to_csv('AnexosITS/SUMARY2.txt', sep = '\t')
    names1.to_csv('AnexosITS/names1.txt', sep = '\t', index = None)
    with open('AnexosITS/dictionaries.json', 'w') as fp:
        json.dump([variedad, procesado, cultivo, tiempo_secado, kit, ota], fp)
    with open('AnexosITS/ssss.json', 'w') as fp:
        json.dump(ssss, fp)
    with open('AnexosITS/dend.json', 'w') as fp:
        json.dump(dend, fp)
...
def box11(data_i, Linaje, Percentage2, Method_i, Metric_i,
    ...
    AAbundancEE.to_csv('AnexosITS/AAbundancEE.txt', sep = '\t')
    names111.to_csv('AnexosITS/names111.txt', sep = '\t', index = None)
    with open('AnexosITS/dictionaries2.json', 'w') as fp:
        json.dump([variedad_i, procesado_i, cultivo_i, tiempo_secado_i, kit_i, ota_i], fp)
    with open('AnexosITS/numeros2.json', 'w') as fp:
        json.dump(numeros2, fp)
    with open('AnexosITS/dendo.json', 'w') as fp:
        json.dump(dendo, fp)
...
def chord_plot(title_kit = ''):
    if title_kit == 'Both':
        ...
        def on_select2():
            ...
            plt.savefig('PlotsITS/Chord_'+title_kit+'_'+data_i.value+'_'+Linaje.value+'_'+str(Percentage2.value)+'_'+datetime.datetime.now().strftime('%d.%B.%Y_%I-%M%p')+'.png', dpi = 900, bbox_inches= 'tight')
            plt.close()
        ###############
        ###############
        ...
        def on_select3():
            ...
            plt.savefig('PlotsITS/Chord_'+title_kit+'_'+data_i.value+'_'+Linaje.value+'_'+str(Percentage2.value)+'_'+datetime.datetime.now().strftime('%d.%B.%Y_%I-%M%p')+'.svg', dpi = 900, bbox_inches= 'tight')
            plt.close()
        ###############
        ###############
...
but_ind_plot_save.style.button_color = 'gold' #'deepskyblue'
...
#### 'ASV':
DATAFRAME_PHYLOGENETIC_ASVs = pd.read_csv('AnexosITS/ASVs_Phylogenetic_Tree.tsv', sep = '\t')
...
if os.path.exists('AnexosITS/unifrac_matrix_ITS_ASVs.txt') == True:
    frame_matrix_otus = pd.read_csv('AnexosITS/unifrac_matrix_ITS_ASVs.txt', sep = '\t')
    id_samples_ASVs = frame_matrix_otus.Sample.tolist()
    unifrac_matrix_ASVs = frame_matrix_otus.iloc[:, 1:].values
else:
    A=urllib.request.urlretrieve('https://raw.githubusercontent.com/eduardo1011/Bioinformatica2019/master/unifrac_matrix_ITS_ASVs.txt',
                            'AnexosITS/unifrac_matrix_ITS_ASVs.txt')
...
#### 'OTU':
DATAFRAME_PHYLOGENETIC_OTUs = pd.read_csv('AnexosITS/OTUs_Phylogenetic_Tree.tsv', sep = '\t')
...
if os.path.exists('AnexosITS/unifrac_matrix_ITS_OTUs.txt') == True:
    frame_matrix_otus = pd.read_csv('AnexosITS/unifrac_matrix_ITS_OTUs.txt', sep = '\t')
    id_samples_OTUs = frame_matrix_otus.Sample.tolist()
    unifrac_matrix_OTUs = frame_matrix_otus.iloc[:, 1:].values
else:
    A=urllib.request.urlretrieve('https://raw.githubusercontent.com/eduardo1011/Bioinformatica2019/master/unifrac_matrix_ITS_OTUs.txt',
                            'AnexosITS/unifrac_matrix_ITS_OTUs.txt')
...
RESULTS_ITS = VBox([blanca, Box([exe], layout = box_exe), blanca, OU])
